...
CREATE TABLE post (
    ...
    created_at DATETIME DEFAULT CURRENT_TIMESTAMP NOT NULL,
    user_id INTEGER NOT NULL REFERENCES user(id) ON DELETE CASCADE,
    image TEXT(255), likes INTEGER DEFAULT 0);
...
CREATE TABLE country (
    id INTEGER PRIMARY KEY AUTOINCREMENT,
    name VARCHAR(100) NOT NULL CHECK (name IN ('Bosnia and Herzegovina', 'Croatia', 'North Macedonia', 'Montenegro', 'Serbia', 'Slovenia', 'Yugoslavia'))
);
...
CREATE TABLE user (
    ...
    college_id INTEGER NOT NULL REFERENCES college(id) ON DELETE CASCADE, location VARCHAR(100) DEFAULT '', bio VARCHAR(150) DEFAULT '');
...
INSERT INTO college (name, state) VALUES
