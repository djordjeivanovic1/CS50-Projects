...
def get_like_count(post_id):
    ...
    like_count = db.execute("SELECT COUNT(*) FROM post_like WHERE post_id = ?", post_id)[0]
...
def get_comments(post_id):
    ...
    comments = db.execute("SELECT content, user_id, created_at FROM post_comment WHERE post_id = ?", post_id)
    ...
    for comment in comments:
        ...
        formatted_comment = {"user_name": user_name, "content": comment["content"], "time": comment["created_at"]}
...
def get_user_profile(user_id):
    ...
    user_data = db.execute("SELECT username, fname, lname, image, email, bio, location, country_id, college_id FROM user WHERE id = ?", user_id)[0]
    answers = db.execute("SELECT answer1, answer2, answer3 FROM profile_questions WHERE user_id = ?", user_id)[0]
    socials = db.execute("SELECT instagram, facebook, linkedin FROM socials WHERE user_id = ?", user_id)[0]
    ...
    user_dict = {
        ...
        "country": db.execute("SELECT name FROM country WHERE id = ?", user_data["country_id"])[0]["name"],
        "college": db.execute("SELECT name FROM college WHERE id = ?", user_data["college_id"])[0]["name"],
...
def update_data_image(user_data):
    ...
    if (user_data["image"] == None):
        user_data["image_url"] = 'https://upload.wikimedia.org/wikipedia/commons/thumb/5/59/User-avatar.svg/1200px-User-avatar.svg.png'

    # Otherwise update the user's image to the one they uploaded
    else:
        user_data["image_url"] = "data:image/png;base64," + base64.b64encode(user_data["image"]).decode('utf-8')
