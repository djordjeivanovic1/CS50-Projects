...
from helpers import login_required, is_strong_password, apology, get_like_count, get_comments, get_username, get_formatted_date, get_user_profile, update_data_image
import base64
...
def register():
    ...
    if request.method == "POST":
        ...
        if not username or not fname or not lname or not email or not password or not confirmation or not country_id or not college_id:
        ...
        if not is_strong_password(password):
            return apology("Password must be at least 8 characters long and include a mix of letters, numbers, and symbols", 400)
        ...
        db.execute("INSERT INTO user (username, fname, lname, email, password, country_id, college_id) VALUES (?, ?, ?, ?, ?, ?, ?)", username, fname, lname, email, hashed_password, country_id, college_id)
...
def edit_profile():
    ...
    if request.method == "POST":
        ...
        db.execute("INSERT OR REPLACE INTO profile_questions (answer1, answer2, answer3, user_id) VALUES (?, ?, ?, ?)", answer1, answer2, answer3, user_id)

        # Update user's socials
        db.execute("INSERT OR REPLACE INTO socials (instagram, facebook, linkedin, user_id) VALUES (?, ?, ?, ?)", instagram, facebook, linkedin, user_id)

        # Update user's profile
        db.execute("UPDATE user SET fname = ?, lname = ?, email = ?, image = ?, bio = ?, location = ? WHERE id = ?", fname, lname, email, request.files["image"].read() if "image" in request.files else None, bio, location, user_id)
    ...
    # User reached route via GET (as by clicking a link or via redirect)
        ...
        return render_template('edit_profile.html', data=get_user_profile(user_id), colleges=colleges, countries=countries)
...
def feed():
    ...
    posts = db.execute("SELECT id, image, caption, likes, created_at, user_id FROM post ORDER BY created_at DESC")
    ...
    return render_template("feed.html", posts=formatted_posts, get_formatted_date=get_formatted_date)
...
def create_post():
    ...
    if request.method == "POST":
        ...
        db.execute("INSERT INTO post (caption, image, created_at, user_id) VALUES (?, ?, ?, ?)", caption, image_data, datetime.datetime.now(), session["user_id"])
...
def like_post(post_id):
    ...
    liked_post = db.execute("SELECT * FROM post_like WHERE post_id = ? AND user_id = ?", post_id, user_id)

    # If not liked, insert a new record into post_like
    if not liked_post:
        db.execute("INSERT INTO post_like (post_id, user_id) VALUES (?, ?)", post_id, user_id)

    # Retrieve current number of likes
    likes = db.execute("SELECT COUNT(*) AS num FROM post_like WHERE post_id = ?", post_id)[0]["num"]
...
def comment_post(post_id):
    ...
    db.execute("INSERT INTO post_comment (post_id, user_id, content) VALUES (?, ?, ?)", post_id, user_id, comment_content)
...
def connect():
    ...
    if request.method == "POST":
        ...
        return render_template("connect.html", people=people, countries=countries, colleges=colleges)
...
def show_all_events():
    ...
    if request.method == "POST":
        ...
        events = db.execute(query, title_query=f"%{title_query}%" if title_query else None)
...
def create_event():
    ...
    if request.method == "POST":
        ...
        db.execute("INSERT INTO event (title, description, date, location) VALUES (?, ?, ?, ?)", title, description, date, location)
...
def show_all_opportunities():
    ...
    if request.method == "POST":
        ...
        opportunities = db.execute(query, title_query=f"%{title_query}%" if title_query else None)
...
def create_opportunity():
    ...
    if request.method == "POST":
        ...
        db.execute("INSERT INTO opportunity (title, description, linked_application, deadline) VALUES (?, ?, ?, ?)", title, description, linked_application, deadline)
